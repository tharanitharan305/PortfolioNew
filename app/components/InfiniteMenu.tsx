...
function createProgram(
  ...
  _transformFeedbackVaryings?: string[] | null,
...
/* ──────────────────────────────────────────────────────────────
 *  Arcball control (mouse / pointer interaction)
 * ────────────────────────────────────────────────────────────── */
...
class ArcballControl {
  ...
  // Bound handlers so we can removeEventListener on dispose
...
class InfiniteGridMenu {
  ...
  private discLocations!: Record<string, number | WebGLUniformLocation | null>;
  ...
  destroy(): void {
    ...
    // Note: WebGL resource cleanup (delete buffers, textures, programs) is
    // optional since losing the context or removing the canvas does it for us.
  }
  ...
  private init(): void {
    ...
    const aMP = gl.getAttribLocation(this.discProgram!, "aModelPosition");
    const aMU = gl.getAttribLocation(this.discProgram!, "aModelUvs");
    const aIM = gl.getAttribLocation(this.discProgram!, "aInstanceMatrix");
    ...
    this.discVAO = makeVertexArray(
      gl,
      [
        [makeBuffer(gl, this.discBuffers.vertices, gl.STATIC_DRAW)!, aMP, 3],
        [makeBuffer(gl, this.discBuffers.uvs, gl.STATIC_DRAW)!, aMU, 2],
      ],
  ...
  private initTexture(): void {
    ...
    // Fill with dark background so empty cells are transparent
      ...
      this.items.map(
        (item) =>
          new Promise<HTMLImageElement>((resolve) => {
            ...
            img.onerror = () => {
              // Fallback: create a coloured placeholder
              ...
              cx.fillText(item.title.charAt(0).toUpperCase(), cellSize / 2, cellSize / 2);
  ...
  private initDiscInstances(count: number): void {
    ...
    const aIM = this.discLocations.aInstanceMatrix as number;
  ...
  private render(): void {
    ...
    gl.uniform1f(this.discLocations.uFrames as WebGLUniformLocation, this._frames);
...
export interface InfiniteMenuProps {
  items?: MenuItem[];
  scale?: number;
  /** Optional — if provided, the component calls this with the active item's data
   *  so a parent can render its own overlay.  When omitted the built-in overlay is used. */
  children?: ReactNode;
}
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("InfiniteMenu initialization failed:", e);
    }
  ...
  const handleButtonClick = () => {
    ...
    if (activeItem.link.startsWith("http")) {
      window.open(activeItem.link, "_blank", "noopener");
    } else {
  ...
  return (
    <div className="infinite-menu-root">
      ...
      {activeItem && (
        <>
          <h2
            className={`infinite-menu-title ${isMoving ? "inactive" : "active"}`}
          >
            {activeItem.title}
          </h2>

          <p
            className={`infinite-menu-description ${isMoving ? "inactive" : "active"}`}
          >
          ...
          {activeItem.link && (
            <div
              ...
              className={`infinite-menu-button ${isMoving ? "inactive" : "active"}`}
            >
              <span className="infinite-menu-button-icon">&#x2197;</span>
            </div>
          )}
        </>
      )}
