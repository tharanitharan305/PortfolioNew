...
// 1×1 transparent pixel — lets useTexture be called unconditionally when
// a front/back image isn't supplied.
const BLANK_PIXEL =
...
interface LanyardProps {
  ...
  lanyardWidth?: number;
  /** CSS height for the wrapper container */
  height?: number | string;
  /** CSS width for the wrapper container */
  width?: number | string;
}
...
function Band({
  ...
  // image isn't supplied for a given face, then skip compositing below.
  const frontTex = useTexture(frontImage || BLANK_PIXEL);
  const backTex = useTexture(backImage || BLANK_PIXEL);

  // Composite the front/back images into the card's texture atlas
  // (front = left half, back = right half). Each image is drawn
  // aspect-preserving (no stretch).
